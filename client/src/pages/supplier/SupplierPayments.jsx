...
import { getSupplierPayments, createSupplierPayment } from "../../services/supplierPayment.service";
...
const SupplierPayments = () => {
  ...
  const handleCreate = async (data) => {
    try {
      ...
      alert(err.response?.data?.message || "Failed to create supplier payment.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
      <div className="wc-page world-module world-payments space-y-5 page-container page-standard">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 secondary-text">Record supplier payments and review payment history.</p>
          </div>
                ...
                {payments.length === 0 ? (
                  ...
                  payments.map((payment, index) => (
                    <tr key={payment.id} className="bg-[var(--surface)] transition-colors hover:bg-[var(--surface-light)]">
                      ...
                      <td className="px-4 py-3 text-sm text-[var(--text)]">{payment.supplier?.supplierName || "—"}</td>
                      <td className="w-32 px-4 py-3 text-sm text-[var(--text)]">{payment.purchase?.purchaseNumber || payment.purchaseId}</td>
                      ...
                      <td className="w-40 px-4 py-3 text-sm text-[var(--text-secondary)]">{payment.paidBy ? `${payment.paidBy.first_name} ${payment.paidBy.last_name}` : "—"}</td>
      ...
      <Modal isOpen={openCreate} onClose={() => setOpenCreate(false)} title="Supplier Payment" size="lg">
        <PaymentForm onSubmit={handleCreate} loading={saving} isCustomer={false} />
      </Modal>
    </>
  );
};
