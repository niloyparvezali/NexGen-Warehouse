...
const SupplierPaymentList = () => {
  ...
  const fetchPayments = async () => {
    try {
      const response = await getSupplierPayments(page, 10, search);
      setPayments(response?.payments ?? []);
      setPagination(response?.pagination ?? {});
    } catch (err) {
      setError(
        err.response?.data?.message || "Failed to load supplier payments.",
      );
    }
  };
  ...
  const handleCreate = async (data) => {
    try {
      ...
      await createSupplierPayment(data);
      ...
      alert(
        err.response?.data?.message || "Failed to create supplier payment.",
      );
    } finally {
      setSaving(false);
    }
  ...
  if (error) {
    return (
      <div className="rounded-lg bg-[var(--danger-soft)] p-4 text-[var(--danger)]">
        {error}
      </div>
    );
  }
      ...
      <div className="space-y-5 page-container page-standard">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[var(--text-secondary)]">
              Record supplier payments and review payment history.
            </p>
          </div>

          <Button onClick={() => setOpenCreate(true)}>Pay Supplier</Button>
        </div>
            ...
            <col className="index-column" />
              ...
              <thead className="bg-[var(--surface-muted)]">
                <tr className="text-left text-xs font-semibold uppercase tracking-[0.08em] text-[var(--text-secondary)]">
                  ...
                  <th className="w-40 px-4 py-3">Paid By</th>
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {payments.length === 0 ? (
                  <tr>
                    <td
                      colSpan="9"
                      className="py-10 px-4 text-center text-[var(--text-secondary)]"
                    >
                  ...
                  payments.map((payment, index) => (
                    <tr
                      key={payment.id}
                      className="bg-[var(--surface)] transition-colors hover:bg-[var(--surface-light)]"
                    >
                      <td className="w-8 px-1 py-3 text-sm text-[var(--text)]">
                        {index + 1}
                      </td>

                      <td className="w-32 px-4 py-3 text-sm text-[var(--text)]">
                      ...
                      <td className="w-32 px-4 py-3 text-sm text-[var(--text)]">
                        {payment.purchase?.purchaseNumber || payment.purchaseId}
                      </td>

                      <td className="w-32 px-4 py-3 text-right text-sm font-medium text-[var(--text)]">
                        ৳ {formatMoney(payment.amount || 0)}
                      </td>

                      <td className="w-28 px-4 py-3 text-sm text-[var(--text)]">
        ...
        <div className="mt-6 flex items-center justify-between">
          <Button
            ...
            onClick={() => hasPreviousPage && setPage(safePage - 1)}
          >
            Previous
          </Button>
          ...
          <Button
            ...
            onClick={() => hasNextPage && setPage(safePage + 1)}
          >
            Next
          </Button>
        </div>
